...
- Definição das permissões que esta tela usa. O RBAC completo é do Documento 0 e a tela que edita perfil × permissão é o documento `02 - manter-perfil-permissao`.
...
| 3 | **Permissões concedidas a ADMIN e USER.** `CARTOES_LISTAR` e `CARTOES_MANTER` ficam nos dois perfis de sistema na carga inicial (Seção 13.1). O que separa um usuário do outro **não** é a permissão, é o escopo por `USU_ID` aplicado no serviço ([RN02](#rn02)). | [PERM01](#perm01), [PERM02](#perm02) |
...
|---|---|---|---|
| <a id="rnf01"></a>RNF01 | Segurança | Cada endpoint exige a autoridade da sua operação (`PERM_CARTOES_LISTAR`, `PERM_CARTOES_MANTER` — ver Seção 13 e [RN01](#rn01)); [EDP07](#edp07) exige apenas usuário autenticado. Além da permissão, o serviço aplica o escopo por `USU_ID` ([RN02](#rn02)). | Teste de acesso com ADMIN, com USER e com um perfil sem as permissões de Cartões. |
...
| <a id="rnf05"></a>RNF05 | Usabilidade | A interface segue o padrão do projeto (Thymeleaf + Tabler + DataTables + AJAX) e é responsiva. O cadastro e a edição são feitos num modal único. | Revisão visual do protótipo. |
...
| <a id="qdd1-5"></a>5 | BOTÃO NOVO CARTÃO | Tipo: Botão (primário)<br>Texto: Novo cartão<br>Ícone: plus | Visível a quem tem [PERM02](#perm02). Ao clicar, executar [RT04](#rt04). |
...
| <a id="qdd1-14"></a>14 | ÍCONES DE AÇÃO | Tipo: Coluna<br>Editar (ícone: edit, tooltip: Editar cartão)<br>Excluir (ícone: trash, tooltip: Excluir cartão) | Visível a quem tem [PERM02](#perm02). Editar → [RT05](#rt05). Excluir → [RT07](#rt07); oculto quando o cartão já está excluído. |
...
| <a id="qdd3-5"></a>5 | CAMPO – LIMITE | Tipo: Input moeda (BRL)<br>Obrigatório: Não | Grava `CACR_LIMITE` (`DECIMAL(15,2)`). Informativo nesta versão ([Observação 11](#2-observações)). |
...
| <a id="qdd3-9"></a>9 | CAMPO – ATIVO | Tipo: Toggle (Sim/Não)<br>Valor default: Sim<br>Exibição: só no modo edição | Grava `CACR_FL_ATIVO`. Ver [RN09](#rn09). |
...
| <a id="qdd3-11"></a>11 | BOTÃO SALVAR | Tipo: Botão (primário)<br>Texto: Salvar<br>Endpoint: [EDP04](#edp04) (criação) ou [EDP05](#edp05) (edição) | Ao clicar, executar [RT06](#rt06). |
...
| <a id="rt04"></a>RT04 | Ao clicar em "Novo cartão" ([ID5](#qdd1-5)) — visível só com [PERM02](#perm02) —, abrir o modal ([QUADRO_DESCRITIVO_3](#quadro-descritivo-3)) em modo criação: campos vazios, Bandeira e Conta de débito sem seleção, sem o campo Ativo. Carregar a Conta de débito por [SB02](#sb02). |
| <a id="rt05"></a>RT05 | Ao clicar no ícone Editar ([ID14](#qdd1-14)) — visível só com [PERM02](#perm02) —, chamar [EDP03](#edp03) com o id e abrir o modal em modo edição, com os campos preenchidos. Se [C1](#c1).qtdVinculos > 0, exibir o aviso [ID10](#qdd3-10). Se o id não for do usuário autenticado, o serviço responde 404 e a tela exibe [MSG11](#msg11). |
| <a id="rt06"></a>RT06 | Ao clicar em "Salvar" ([ID11](#qdd3-11)): validar Descrição obrigatória ([MSG02](#msg02)); validar dia de fechamento e dia de vencimento entre 1 e 31 quando preenchidos ([MSG03](#msg03)); validar final do cartão com exatamente 4 dígitos quando preenchido ([MSG04](#msg04)). Em criação, chamar [EDP04](#edp04); em edição, [EDP05](#edp05). Em sucesso, exibir [MSG01](#msg01) (criação) ou [MSG05](#msg05) (edição), fechar o modal e recarregar o grid via [EDP02](#edp02). Conta de débito inválida ([RN07](#rn07)) → [MSG10](#msg10) no campo Conta de débito. |
| <a id="rt07"></a>RT07 | Ao clicar no ícone Excluir ([ID14](#qdd1-14)) — visível só com [PERM02](#perm02) —, exibir a confirmação [MSG06](#msg06). Ao confirmar, chamar [EDP06](#edp06). Cartão com faturas ou despesas vinculadas → [MSG08](#msg08), com a oferta de desativar (ao aceitar, chamar [EDP05](#edp05) apenas com `CACR_FL_ATIVO = FALSE`). Em sucesso da exclusão, exibir [MSG07](#msg07) e recarregar o grid. |
...
|---|---|---|---|---|
| <a id="edp01"></a>EDP01 | GET | [PERM01](#perm01) | /cartoes/listar | N |
| Retorna a página da listagem de cartões (Thymeleaf). O grid é carregado por [EDP02](#edp02). | | | | |
| <a id="edp02"></a>EDP02 | GET | [PERM01](#perm01) | /cartoes/listar-dados | N |
...
| <a id="edp03"></a>EDP03 | GET | [PERM02](#perm02) | /cartoes/buscar/{id} | N |
...
| <a id="edp04"></a>EDP04 | POST | [PERM02](#perm02) | /cartoes/inserir | N |
...
| <a id="edp05"></a>EDP05 | PUT | [PERM02](#perm02) | /cartoes/editar/{id} | N |
...
| <a id="edp06"></a>EDP06 | DELETE | [PERM02](#perm02) | /cartoes/excluir/{id} | N |
...
| ID | DESCRIÇÃO |
|---|---|
| <a id="rn01"></a>RN01 | Cada endpoint exige a autoridade da sua operação: [EDP01](#edp01)/[EDP02](#edp02) → `PERM_CARTOES_LISTAR`; [EDP03](#edp03)/[EDP04](#edp04)/[EDP05](#edp05)/[EDP06](#edp06) → `PERM_CARTOES_MANTER`. [EDP07](#edp07) exige apenas usuário autenticado. As autoridades são resolvidas pelo `getAuthorities()` do `Usuario` a partir do perfil e das permissões vinculadas em `PERFIL_PERMISSAO`. `CARTOES_MANTER` pressupõe `CARTOES_LISTAR` (sem listar não há tela). |
...
Duas do módulo **Cartões de Crédito** (`PERM_MODULO = 'Cartões de Crédito'`). Fazem parte do catálogo do código e da carga inicial (Documento 0, [QUADRO_DESCRITIVO_26](../00%20-%20analise-geral/documento-0-fundacao.md#quadro-descritivo-26)). Convenção domínio-primeiro; cada uma vira a autoridade `PERM_{CÓDIGO}`.
...
Os dois perfis de sistema recebem as duas permissões na carga inicial. Um perfil personalizado que não as receba (montado no editor de perfil do documento `02`) não enxerga o menu 'Cartões' nem acessa qualquer endpoint desta tela; ainda assim, se tiver acesso à tela de Despesa, consome os próprios cartões ativos por [EDP07](#edp07), que exige apenas autenticação.
...
|---|---|---|
| <a id="perf01"></a>PERF01 | ADMIN | Administrador do sistema. `PERF_FL_SISTEMA = TRUE`. Recebe todas as permissões na carga inicial, inclusive as duas de Cartões de Crédito (Seção 13.1). Mantém **apenas os próprios cartões** ([RN02](#rn02)). Corresponde a `ROLE_ADMIN`. |
| <a id="perf02"></a>PERF02 | USER | Usuário comum. `PERF_FL_SISTEMA = TRUE`. Recebe `CARTOES_LISTAR` e `CARTOES_MANTER` na carga inicial. Mantém apenas os próprios cartões. Corresponde a `ROLE_USER`. |
...
- Permissões `CARTOES_LISTAR` / `CARTOES_MANTER` concedidas a **ADMIN e USER** na carga inicial (matriz 13.1 ✓ nas duas colunas). O que separa os usuários é o escopo, não a permissão.
